var ${component.name}_record = Ext.data.Record.create([
	 ...
	 {name:'${childColumnsItem.name}'
	  <#if childColumnsItem.type?exists>,type:'${childColumnsItem.type}'
	  </#if><#if childColumnsItem.mapping?exists>,mapping:'${childColumnsItem.mapping}'
	  </#if><#if childColumnsItem.convert?exists>,convert:'${childColumnsItem.convert}'
	  </#if><#if childColumnsItem.dateFormat?exists>,dateFormat:'${childColumnsItem.dateFormat}'
	  </#if><#if childColumnsItem.defaultValue?exists>,defaultValue:'${childColumnsItem.defaultValue}'
	  </#if>}
	 </#list>
      </#if>
  ]);
function ${component.name}_doAdd (){
	var ${component.name}_new_record = new ${component.name}_record({
	<#if component.childColumns?has_content> 
		<#list component.childColumns as childColumnsItem>
			<#if childColumnsItem_index != 0>,</#if>
				${childColumnsItem.name}:
				<#if childColumnsItem.value?has_content>
					<#if ! childColumnsItem.type?has_content || "string" == childColumnsItem.type>
						'${childColumnsItem.value}'
					<#else>
						${childColumnsItem.value}
					</#if>
				<#else>
					<#if ! childColumnsItem.type?has_content>''
					<#elseif "string" = childColumnsItem.type>''
					<#elseif "int" = childColumnsItem.type>0
					<#elseif "float" = childColumnsItem.type>0.0
					<#elseif "boolean" = childColumnsItem.type>true
					<#elseif "date" = childColumnsItem.type>(new Date()).clearTime()
					<#else>''
					</#if>
				</#if>
		</#list>
	...
	${component.name}.stopEditing();
    ${component.name}.store.insert(0, ${component.name}_new_record);
    ${component.name}.startEditing(0, 0);
}
...
var ${component.name}_cm = new Ext.grid.ColumnModel([
      new Ext.grid.RowNumberer(),
       ...
       <#if component.childColumns?has_content> 
         <#list component.childColumns as childColumnsItem>
	 	 ,{dataIndex:'${childColumnsItem.name?replace('.', '___')}'

	 	<#------- ,{dataIndex:'${childColumnsItem.dataIndex?replace('.', '___')}'--------------->
             ...
             </#if><#if childColumnsItem.editor?exists>,editor:${childColumnsItem.editor}
...
${component.name}_cm.defaultSortable = true;



var ${component.name}=new Ext.grid.EditorGridPanel({
		id:'${component.name}' ,
...
<#-------com.cutty.bravo.core.ui.component.Component------------------->
		...
		<#if component.ctCls?exists>,ctCls:'${component.ctCls}'
                ...
                </#if><#if component.autoEncode?exists>,autoEncode:${component.autoEncode}
		</#if><#if component.clicksToEdit?exists>,clicksToEdit:${component.clicksToEdit}
		</#if><#if component.selModel?exists>,selModel:${component.selModel}
		...
		</#if><#if component.entityKey?exists>,entityKey:${component.entityKey}
		</#if> 
		,doAddHandler:${component.name}_doAdd
});
