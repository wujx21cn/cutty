	...
	<@bravo.Viewport  layout="border">
		<@bravo.FormPanel name="workflowAddForm" region="center" border="false" width="600" tbar="toolBar" dataProxy="../workflow/task!taskProcess.action">  
			<@bravo.Toolbar name="toolBar" valign="bottom">
				<@bravo.Button tooltip="'接受任务'" text="接受任务" iconCls="add" handler="submitAndCreateProcess(\\'workflowAddForm\\')"/>
			</@bravo.Toolbar>
...
function submitAndCreateProcess(formPanelName){
    ...
    formPanel.form.doAction('submit', {
		...
		method : 'post',
		params : {jsonFormData:'Y',workflow_definition_name:'${workflow_definition_name}',workflow_operation:'workflow_operation_accept',workflow_entity_name:'${workflow_entity_name}',workflow_entity_id:'${workflow_entity_id}',workflow_task_id:'${workflow_task_id}'},
