...
Ext.override(QoDesk.QoPreferences, {
    ...
    save : function(params){    	
	    ...
	    Ext.Ajax.request({
			url: bravoHome + '/common/deskTopConfig!saveConfig.action',
			...
			success: function(o){
				if(o && o.responseText && Ext.decode(o.responseText).success){
					...
					saveComplete('错误', '服务器处理出错.');
				}
			},
			failure: function(){
				saveComplete('错误', '服务器连接出错.');
			},
			scope: this
		});
	...
	QoDesk.QoPreferences.NavPanel.superclass.constructor.call(this, {
		...
		html: '<ul id="pref-nav-panel"> \
				<li> \
					...
					<a id="viewShortcuts" href="#">快捷方式</a><br /> \
					<span>选择用于在桌面显示的快捷方式.</span> \
				</li> \
				<li> \
					<img src="'+Ext.BLANK_IMAGE_URL+'" class="icon-pref-autorun"/> \
					<a id="viewAutoRun" href="#">自动启动</a><br /> \
					<span>选择用于在登陆后自动开启的页面.</span> \
				</li> \
				<li> \
					<img src="'+Ext.BLANK_IMAGE_URL+'" class="icon-pref-quickstart"/> \
					<a id="viewQuickstart" href="#">快速启动</a><br /> \
					<span>选择用于在快速启动栏中显示快捷方式.</span> \
				</li> \
				<li> \
					<img src="'+Ext.BLANK_IMAGE_URL+'" class="icon-pref-appearance"/> \
					<a id="viewAppearance" href="#">颜色外观</a><br /> \
					<span>调整窗口颜色及外观主题.</span> \
				</li> \
				<li> \
...
QoDesk.QoPreferences.AutoRun = function(config){
	...
	QoDesk.QoPreferences.AutoRun.superclass.constructor.call(this, {
		...
		loader:  new Ext.tree.TreeLoader({dataUrl:'../common/deskTopConfig!viewAutoRun.action?'
	...
	function onDoubleClick(node, o){
		if (node.leaf && node.id)
		{
			if(this.getChecked().indexOf(node) >= 0){ 
...
QoDesk.QoPreferences.Shortcuts = function(config){
	...
	QoDesk.QoPreferences.Shortcuts.superclass.constructor.call(this, {
		...
		loader:  new Ext.tree.TreeLoader({dataUrl:'../common/deskTopConfig!viewShortCuts.action?'
		,baseParams: {contextDataName:'treeData',jsonCheckedTreeData:'Y'} 
		,baseAttrs: {uiProvider: Ext.tree.TreeCheckNodeUI}
		}),
		rootVisible: false,
...
QoDesk.QoPreferences.QuickStart = function(config){
    ...
    QoDesk.QoPreferences.QuickStart.superclass.constructor.call(this, {
		...
		loader:  new Ext.tree.TreeLoader({dataUrl:'../common/deskTopConfig!viewQuickStart.action?'
		,baseParams: {contextDataName:'treeData',jsonCheckedTreeData:'Y'} 
		,baseAttrs: {uiProvider: Ext.tree.TreeCheckNodeUI}
		}),
		rootVisible: false,
...
QoDesk.QoPreferences.Appearance = function(config){
	...
	var store = new Ext.data.JsonStore({
		...
		id: 'id',
		root: 'images',
		url: './deskTopConfigResource!findAndRendThemesJsonData.action'
	});
...
QoDesk.QoPreferences.Background = function(config){
	...
	var store = new Ext.data.JsonStore({
		...
		id: 'id',
		root: 'images',
		url: './deskTopConfigResource!findAndRendWallpapersJsonData.action'
	});
