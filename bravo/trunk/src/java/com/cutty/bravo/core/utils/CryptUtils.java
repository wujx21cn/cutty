/* com.cutty.bravo.core.utils.CryptUtils.java
...
{{IS_NOTE
	...
	History:
		2011-6-15 下午02:18:11, Created by Jason.Wu
}}IS_NOTE
 ...
 * 
 * <p>
 * <a href="CryptUtils.java.html"><i>View Source</i></a>
 * </p>
 * 
...
public class CryptUtils {
	...
	public static byte[] encrypt(byte[] src, byte[] key)throws RuntimeException {
		// DES算法要求有一个可信任的随机数源
		try {
			SecureRandom sr = new SecureRandom(); // 从原始密匙数据创建DESKeySpec对象
	...
	public static byte[] decrypt(byte[] src, byte[] key) {
		try { // DES算法要求有一个可信任的随机数源
			...
			// 用密匙初始化Cipher对象
			cipher.init(Cipher.DECRYPT_MODE, securekey, sr); // 现在，获取数据并解密 //
																// 正式执行解密操作
			return cipher.doFinal(src);
		} catch (Exception e) {
	...
	/**
	 * 数据解密 * @param data * @param key 密钥 * @return
	 * 
	 * @throws Exception
	 */
	...
	public static void main(String[] args) throws Exception{
		String resultsss = CryptUtils.encrypt("F0-DE-F1-1A-DA-B0[2011-06-20]", "IFUCKYOU,shitibm");
		System.out.println(resultsss);
		System.out.println(CryptUtils.decrypt(resultsss, "IFUCKYOU,shitibm"));
	}
}