...
public class ResolverUtil<T> {
    ...
    //需要mapping的jar，通过正则表达式匹配
    private List<String> mappingClassPathJars;
    
    public ResolverUtil(){};
    public ResolverUtil(List<String> mappingClassPathJars){
    ...
    public void findInPackage(Test test, String packageName) {
        ...
        URL[] sysytemUrls = ((URLClassLoader)systemClassLoader).getURLs(); 
        if (null != mappingClassPathJars)
        for (int i=0;i<sysytemUrls.length;i++){
        	for (String mappingClassPathJar:mappingClassPathJars){
        		if (Pattern.matches(mappingClassPathJar,sysytemUrls[i].toExternalForm()))urls.add(sysytemUrls[i]);
        	}
        	
        	
        }
        try {
        	Enumeration<URL> resUrls = loader.getResources(packageName);
