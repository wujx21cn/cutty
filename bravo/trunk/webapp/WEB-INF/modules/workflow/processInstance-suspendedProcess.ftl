...
<@bravo.Page name="ProcessInstanceManager" title="已中止流程管理">
	<@bravo.Viewport  layout="border">
		<@bravo.FormPanel name="processInstance_find" tbar="Toolbar" region="north"  frame="true" autoScroll="true" height="130" width="750"  title="已中止流程列表" collapsible="true"  >  
			...
			<@bravo.FieldSet labelWidth="105" cols="1,1,1;1,1,1" rows="1,1,1;1,1,1" >		
				...
				<@bravo.DateTime fieldLabel="创建时间早于" name="create_index_1"  endDateField="create_index_2" width="120" linkAgeField="create_index_2"/>
				<@bravo.DateTime fieldLabel="创建时间晚于" name="create_index_2"  startDateField="create_index_1" width="120"/>	
		...
		<@bravo.GridPanel  region="center"  stripeRows="true" name="processInstance_grid"  dataProxy="./processInstance!findProcess.action?processInstance_restriction=isNotNull&processInstance.isSuspended=1&processInstance.end_restriction=isNull"  contextDataName="processInstances" collapsible="true" >  
...
function  initCalder(dateObject){
	var start = Ext.getCmp(dateObject.startDateField);
	if (start) {
		if (null != start.value && "" != start.value){
			var startDate = dateObject.parseDate(start.value);
			dateObject.enable();
			dateObject.setMinValue(startDate);
		} else {
			dateObject.enable();
			dateObject.setMinValue(null);
		}
	} 
	...
	if (end) {
		if (null != end.value && "" != end.value){
			var endDate = dateObject.parseDate(end.value);
			dateObject.enable();
			dateObject.setMaxValue(endDate);
		} else {
			dateObject.enable();
			dateObject.setMaxValue(null);
		}
	}
	...
	if(dateObject.menu == null){
		dateObject.menu = new Ext.menu.DateMenu();
	}
	Ext.apply(dateObject.menu.picker,  {
		minDate : dateObject.minValue,
		maxDate : dateObject.maxValue,
		disabledDatesRE : dateObject.ddMatch,
		disabledDatesText : dateObject.disabledDatesText,
		disabledDays : dateObject.disabledDays,
		disabledDaysText : dateObject.disabledDaysText,
		format : dateObject.format,
		showToday : dateObject.showToday,
		minText : String.format(dateObject.minText, dateObject.formatDate(dateObject.minValue)),
		maxText : String.format(dateObject.maxText, dateObject.formatDate(dateObject.maxValue))
	});
	dateObject.menu.on(Ext.apply({}, dateObject.menuListeners, {
		scope:dateObject
	}));
	dateObject.menu.picker.setValue(dateObject.getValue() || new Date());
	dateObject.menu.show(dateObject.el, "tl-bl?");
	if(!dateObject){
		return;
	}
}
